...
export function* generateGitDateRangePrev(startDate, endDate, interval = 7) {
  let { startDateString, endDateString } = getGitHubDateRangePrev(
    interval,
    startDate
  );
  while (new Date(endDateString).getTime() >= new Date(endDate).getTime()) {
    yield { startDateString, endDateString };
    ({ startDateString, endDateString } = getGitHubDateRangePrev(
      interval,
      endDateString
    ));
  }
...
export function* generateGitDateRangeNext(startDate, endDate, interval = 7) {
  let { startDateString, endDateString } = getGitHubDateRangeNext(
    interval,
    startDate
  );
  while (new Date(endDateString).getTime() <= new Date(endDate).getTime()) {
    yield { startDateString, endDateString };
    ({ startDateString, endDateString } = getGitHubDateRangeNext(
      interval,
      endDateString
    ));
  }
